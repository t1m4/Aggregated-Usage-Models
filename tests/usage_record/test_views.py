...
class TestUsageRecordPriceLimitView:
    ...
    def test_view_with_one_response(self, api_client):
        price = 10
        DataUsageRecordFactory.create(price=price)
        DataUsageRecordFactory.create(price=price + 10)
    ...
    def test_view_with_filtering(self, api_client):
        price = 10
        DataUsageRecordFactory.create(price=price)
        VoiceUsageRecordFactory.create(price=price)
...
class TestUsageRecordTotalMetricsView:
    def test_view_with_empty_response(self, api_client):
        usage_record = DataUsageRecordFactory.create()
        usage_metrics_url = reverse("usage-metrics", kwargs={"subscription_id": usage_record.subscription_id_id + 1})
    ...
    def test_view_with_two_response(self, api_client):
        usage_record = DataUsageRecordFactory.create()
        usage_metrics_url = reverse("usage-metrics", kwargs={"subscription_id": usage_record.subscription_id_id})
        response = api_client.get(usage_metrics_url)
        response_json = response.json()
        assert len(response_json) == 1
        record = response_json[0]
        assert record["subscription_id"] == usage_record.subscription_id_id
        assert record["total_price"] == usage_record.price
        assert record["total_used"] == usage_record.kilobytes_used
